...
export function pickFrom(items, rng) {
 ...
 */
export function pickWeighted(items, rng, bias = 2.5) {
    const idx = Math.min(Math.floor(Math.pow(rng(), 1 / bias) * items.length), items.length - 1);
    return items[idx];
}
